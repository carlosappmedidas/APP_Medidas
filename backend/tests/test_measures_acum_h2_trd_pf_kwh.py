# tests/test_measures_acum_h2_trd_pf_kwh.py
...
from app.measures.services import procesar_acum_h2_trd_pf_kwh
...
def test_acum_h2_trd_pf_kwh_crea_medida_general():
    ...
    fichero = IngestionFile(  # type: ignore[call-arg]
        ...
        anio=2025,
        mes=5,
        filename="ACUM_H2_TRD_0277_P1_202505.0",
        storage_key="local-test/ACUM_H2_TRD_0277_P1_202505.0",
        status="pending",
        uploaded_by=user.id,
    )
    db.add(fichero)
    ...
    mg = procesar_acum_h2_trd_pf_kwh(
        db=db,
        tenant_id=tenant.id,
        empresa_id=empresa.id,
        fichero=fichero,
...
def test_acum_h2_trd_pf_kwh_acumula_con_segundo_fichero():
    db = crear_sesion_sqlite()
    tenant, empresa, user = _crear_tenant_empresa_usuario(db)
    ...
    mg1 = procesar_acum_h2_trd_pf_kwh(
        db=db,
        tenant_id=tenant.id,
        empresa_id=empresa.id,
        fichero=fichero1,
    ...
    fichero2 = IngestionFile(  # type: ignore[call-arg]
        ...
        anio=2025,
        mes=5,
        filename="ACUM_H2_TRD_0277_P1_202505_v2.0",
        storage_key="local-test/ACUM_H2_TRD_0277_P1_202505_v2.0",
        status="pending",
        uploaded_by=user.id,
    )
    db.add(fichero2)
    ...
    mg2 = procesar_acum_h2_trd_pf_kwh(
        db=db,
        tenant_id=tenant.id,
        empresa_id=empresa.id,
        fichero=fichero2,
