// app/components/settings/PublicacionesSettingsSection.tsx
// Sección de Configuración para la feature "Descarga de Publicaciones REE":
// lista las conexiones FTP activas del tenant y permite editar su campo `carpeta_publicaciones`.
...
interface FtpConfig {
  ...
  carpeta_publicaciones: string | null;
  usar_tls: boolean;
  activo: boolean;
}

// Tipos del nuevo bloque "Automatización" — espejo del backend.
...
export default function PublicacionesSettingsSection({ token }: Props) {
  ...
  const cargarConfigs = useCallback(async () => {
    ...
    try {
      ...
      for (const c of data) {
        if (c.activo) initial[c.id] = c.carpeta_publicaciones ?? "";
      }
      setDrafts(initial);
    } catch (e: unknown) {
  ...
  const cargarAutoConfig = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch(`${API_BASE_URL}/measures/descarga/automatizacion/config`, {
        headers: getAuthHeaders(token),
      });
      if (!res.ok) return;
      const data = await res.json();
      const c = data?.buscar_publicaciones_ree;
      setAutoConfig({
        activa:         !!c?.activa,
  ...
  const handleToggleAuto = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/measures/descarga/automatizacion/config/buscar_publicaciones_ree`,
        {
          method: "PATCH",
  ...
  const handleRevisarAhora = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/measures/descarga/automatizacion/revisar-ahora/buscar_publicaciones_ree`,
  ...
  const handleSave = async (id: number) => {
    ...
    // El backend aplica `if carpeta_publicaciones is not None` → "" se persiste en BD.
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/ftp/configs/${id}`, {
        method: "PATCH",
        headers: { ...getAuthHeaders(token), "Content-Type": "application/json" },
        body: JSON.stringify({ carpeta_publicaciones: valueToSend }),
      });
      if (!res.ok) {
      ...
      setConfigs(prev => prev.map(c => c.id === id ? { ...c, carpeta_publicaciones: updated.carpeta_publicaciones } : c));
      // Normalizar draft al valor persistido — "" y null se tratan igual visualmente
      setDrafts(prev => ({ ...prev, [id]: updated.carpeta_publicaciones ?? "" }));
      setSavedId(id);
  ...
  return (
    <div className="text-sm" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{
        ...
        Indica la ruta SFTP donde cada conexión almacena los ficheros <strong style={{ color: "var(--text)" }}>publicados por REE</strong> (BALD, M1, PS).
        ...
        {". "}Ejemplo: <code style={{ padding: "1px 5px", background: "var(--card-bg)", borderRadius: 4, fontSize: 10 }}>/01/entradaHistorico</code>
        {". "}Deja el campo vacío si esta conexión no se usa para descargar publicaciones REE.
        <br />
        <span style={{ fontSize: 10, marginTop: 4, display: "inline-block" }}>
          ℹ️ Las conexiones se dan de alta en <strong style={{ color: "var(--text)" }}>Comunicaciones → Conexiones FTP</strong>. Aquí solo configuras su carpeta de publicaciones.
        </span>
      </div>
        ...
        <table className="ui-table text-[11px]">
          <thead className="ui-thead">
            <tr>
              ...
              <th className="ui-th">Carpeta Publicaciones</th>
          ...
          <tbody>
            {loading ? (
              ...
              const changed = hasChanges(c.id, c.carpeta_publicaciones);
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/* TARJETA AUTOMATIZACIÓN · Búsqueda de publicaciones REE                  */}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      <UiCard
        variant="nested"
        padding="none"
        radius="md"
        style={{ border: "0.5px solid var(--card-border)", padding: "16px 18px", marginTop: 4 }}
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text)" }}>
                Automatización · Búsqueda de publicaciones REE
                <span style={{ marginLeft: 8, fontSize: 10, color: "#378ADD", letterSpacing: "0.04em" }}>· 22:00</span>
              </div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                Cada día a las 22:00 revisa el calendario REE y avisa con una alerta si hay BALDs nuevos en SFTP para los hitos M2 / M7 / M11 / ART15 (próximos 3 días).
              </div>
            </div>
          </div>
          <button
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text)" }}>
              {autoConfig?.ultimo_run_at ? (() => {
                // UTC → Madrid si el ISO no trae zona (mismo patrón que Objeciones).
