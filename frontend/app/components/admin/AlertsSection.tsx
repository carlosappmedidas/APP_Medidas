...
export default function AlertsSection({ token, currentUser }: Props) {
  ...
  // ── Cargar comentarios de la alerta seleccionada ──────────────────────
  ...
  const handleRecalculateAll = async () => {
    ...
    try {
      ...
      await loadAlerts();
    } catch { setError("No se pudo recalcular todo el tenant."); }
    finally { setRecalculatingAll(false); }
  };

  // ── Agrupación de alertas por categoría ──────────────────────────────
  ...
  // ── Años y meses únicos de las alertas ─────────────────────────────────
  const aniosUnicos = useMemo(() => [...new Set(alerts.map((a) => a.anio))].sort((a, b) => b - a), [alerts]);
  const mesesUnicos = useMemo(() => [...new Set(alerts.map((a) => a.mes))].sort((a, b) => a - b), [alerts]);
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td colSpan={isAbs ? 8 : 9} style={{ ...tdStyle, textAlign: "center", color: "var(--text-muted)" }}>
            ...
            {rows.map((a) => {
              ...
              const rowBg = isSelected ? "rgba(37,99,235,0.08)" : undefined;
              return (
                <React.Fragment key={a.id}>
                  <tr style={{ background: rowBg, cursor: "pointer" }} onClick={() => handleSelectAlert(a)}>
                    <td style={tdStyle}>
                      <span style={{ fontWeight: 500 }}>{a.empresa_nombre}</span>
                    </td>
                    <td style={tdStyle}>
                      {MESES[(a.mes ?? 1) - 1]} {a.anio}
                    </td>
                    ...
                    <td style={{ ...tdStyle, textAlign: "right" }}>
                      {isAbs
                        ? fmt(a.threshold_value, a.diff_unit)
                        : fmt(a.diff_value, a.diff_unit)}
                    </td>
                    <td style={tdStyle}>
                  ...
                  {/* Panel de detalle inline bajo la fila seleccionada */}
                  {isSelected && (
                    <tr>
                      <td colSpan={isAbs ? 8 : 9} style={{ padding: 0, background: "var(--card-bg)" }}>
                        {renderDetail(a)}
                      </td>
                    </tr>
                  )}
  ...
  const renderDetail = (a: AlertRow) => (
    <div style={{
      ...
      {/* Cambio de estado — solo si puede gestionar */}
      {canManage && (
        <div style={{ marginBottom: 12 }}>
          ...
          {canManage && (
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 6 }}>
              El comentario del campo de abajo se adjuntará al cambio de estado.
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
        <textarea
          style={{
            ...inputStyle, flex: 1, resize: "vertical", minHeight: 60,
            fontFamily: "inherit",
          }}
    ...
    return (
      ...
      >
        ...
        <span style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 500 }}>
          {CATEGORY_LABELS[cat] === "Mes anterior"
            ? "Variación respecto al mes anterior"
            : cat === "absoluta"
              ? "Valor directamente fuera de rango"
        ...
        <div style={{
          ...
          {canManage && (
            <div style={{ display: "flex", gap: 6 }}>
              ...
              >
                {recalculatingAll ? "Recalculando..." : "Recalcular todo el tenant"}
              </button>
            </div>
          )}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 8, padding: "12px 16px" }}>
          {[
            ...
            <div key={s.label} style={{
              background: "var(--field-bg-soft)", borderRadius: 8, padding: "10px 12px",
            }}>
        ...
        <div style={{
          ...
          {[
            ...
            },
            {
              ...
              options: [
                { value: "all", label: "Todos" },
                ...mesesUnicos.map((m) => ({ value: String(m), label: MESES[m - 1] ?? String(m) })),
              ],
            },
            {
