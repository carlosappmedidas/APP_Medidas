...
const CATEGORY_COLORS: Record<Category, { variant: UiChipVariant; border: string }> = {
  mes_anterior:  { variant: "info",    border: "rgba(37,99,235,0.3)" },
  absoluta:      { variant: "warning", border: "rgba(245,158,11,0.3)" },
  anio_anterior: { variant: "success", border: "rgba(5,150,105,0.3)" },
};
const SEVERITY_VARIANTS: Record<Severity, UiChipVariant> = {
  info:     "muted",
  warning:  "warning",
  critical: "danger",
};
const LIFECYCLE_VARIANTS: Record<LifecycleStatus, UiChipVariant> = {
  nueva:       "danger",
  en_revision: "info",
  resuelta:    "success",
};
              ...
              return (
                <React.Fragment key={a.id}>
                  <tr style={{ background: isSelected ? "rgba(37,99,235,0.08)" : undefined, cursor: "pointer" }} onClick={() => handleSelectAlert(a)}>
                    ...
                    <td style={tdStyle}><UiChip variant={SEVERITY_VARIANTS[a.severity]} size="sm">{a.severity === "critical" ? "Crítica" : a.severity === "warning" ? "Warning" : "Info"}</UiChip></td>
                    <td style={tdStyle}><UiChip variant={LIFECYCLE_VARIANTS[a.lifecycle_status]} size="sm">{LIFECYCLE_LABELS[a.lifecycle_status]}</UiChip></td>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {(["nueva", "en_revision", "resuelta"] as LifecycleStatus[]).map((s) => {
              ...
              // Colores específicos para botones de lifecycle (no son chips)
              const btnColors: Record<LifecycleStatus, { bg: string; color: string }> = {
            ...
            {comments.map((c) => {
              ...
              return (
                <div key={c.id} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  {(() => {
                    const dotColors: Record<LifecycleStatus, string> = {
                      nueva: "#f87171", en_revision: "#60a5fa", resuelta: "#34d399",
                    };
                    return (
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: ls ? dotColors[ls] : "var(--text-muted)", marginTop: 4, flexShrink: 0 }} />
                    );
                  })()}
                  <div>
                    <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                      {c.user_email ?? "Sistema"} · {fmtDatetime(c.created_at)}
                      {ls && <> · <UiChip variant={LIFECYCLE_VARIANTS[ls]} size="sm">{LIFECYCLE_LABELS[ls]}</UiChip></>}
                    </div>
