// Filtros del panel "Descarga de Publicaciones REE".
// Clon del de Objeciones con los mismos filtros: empresa, periodo, fecha publicación, nombre.

"use client";

import { useEffect, useMemo, useRef, useState } from "react";
import type { EmpresaOption } from "./types";
import { IconSearch } from "../objeciones/shared/icons";
...
export default function DescargaFiltros({
  ...
  const toggleEmpresa = (id: number) => {
    if (empresaIds.includes(id)) setEmpresaIds(empresaIds.filter((x) => x !== id));
    else setEmpresaIds([...empresaIds, id]);
  };
  ...
  const clearBtnStyle: React.CSSProperties = { fontSize: 10, padding: "2px 6px", height: 20, opacity: 0.7, cursor: "pointer" };
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 14, marginBottom: 14 }}>
          ...
          {dropdownOpen && (
            <div style={{
              ...
              <div style={{
                ...
                <button type="button" className="ui-btn ui-btn-ghost ui-btn-xs" onClick={seleccionarTodas} style={{ fontSize: 10 }}>Todas</button>
                <button type="button" className="ui-btn ui-btn-ghost ui-btn-xs" onClick={limpiarSeleccion} style={{ fontSize: 10 }}>Limpiar</button>
              </div>
              {empresas.length === 0 ? (
                ...
                empresas.map((emp) => {
                  ...
                  return (
                    <label key={emp.id} style={{
                      ...
                      <span style={{ flex: 1 }}>{emp.nombre || `Empresa ${emp.id}`}</span>
                      {emp.codigo_ree && (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <select className="ui-select" value={periodoAño} onChange={(e) => setPeriodoAño(e.target.value)}
          style={{ ...selectStyle, minWidth: 72 }} title="Año del periodo">
          <option value="">Año</option>
          {AÑOS.map((y) => <option key={y} value={String(y)}>{y}</option>)}
        </select>
        <select className="ui-select" value={periodoMes} onChange={(e) => setPeriodoMes(e.target.value)}
          style={{ ...selectStyle, minWidth: 110 }} title="Mes del periodo">
          <option value="">Mes</option>
          {MESES.map((m) => <option key={m.val} value={m.val}>{m.label}</option>)}
        </select>
        {(periodoAño || periodoMes) && (
          <button type="button" className="ui-btn ui-btn-ghost" onClick={limpiarPeriodo}
            style={clearBtnStyle} title="Limpiar periodo">✕</button>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <input type="date" className="ui-input" value={fechaDesde}
          onChange={(e) => setFechaDesde(e.target.value)}
          style={inputDateStyle} title="Fecha mínima de publicación SFTP" />
        <span style={{ ...labelStyle, fontSize: 10 }}>Hasta</span>
        <input type="date" className="ui-input" value={fechaHasta}
          onChange={(e) => setFechaHasta(e.target.value)}
          style={inputDateStyle} title="Fecha máxima de publicación SFTP" />
        {(fechaDesde || fechaHasta) && (
          <button type="button" className="ui-btn ui-btn-ghost" onClick={limpiarFechas}
            style={clearBtnStyle} title="Limpiar rango de fechas">✕</button>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 200 }}>
        ...
        <input type="text" className="ui-input" value={nombre}
          onChange={(e) => setNombre(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter") onBuscar(); }}
          placeholder="Texto contenido en el fichero (ej: BALD)"
          style={{ fontSize: 11, padding: "4px 8px", height: 28, flex: 1, minWidth: 160 }} />
      </div>

      <button type="button" onClick={onBuscar} disabled={loading}
