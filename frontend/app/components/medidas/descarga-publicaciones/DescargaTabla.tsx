// Tabla de resultados del panel "Descarga de Publicaciones REE".
// Reutiliza el badge de estado del módulo de objeciones (es genérico).

"use client";

import { useEffect, useMemo, useState } from "react";
import type { BusquedaResult } from "./types";
import { BadgeEstadoDescarga } from "../objeciones/shared/badges";
...
export default function DescargaTabla({
  ...
  useEffect(() => { setPaginaActual(1); }, [resultados.length]);
  ...
  const toggleAll = () => {
    ...
    else setSeleccionados(allSelKeys);
  };
        ...
        <table className="ui-table text-[11px]" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
            ...
            <tr>
              <th className="ui-th" style={{ width: 36, textAlign: "center" }}>
                <input type="checkbox" checked={allSelected}
                  ...
                  onChange={toggleAll} disabled={seleccionables.length === 0}
          ...
          <tbody>
            {loading ? (
              ...
              <tr className="ui-tr">
                <td colSpan={9} className="ui-td text-center ui-muted" style={{ padding: "32px 16px" }}>
                  Sin ficheros publicados encontrados para los filtros indicados.
                </td>
              </tr>
            ) : (
                  ...
                  <tr key={k} className="ui-tr" style={rowStyle}>
                    <td className="ui-td" style={{ width: 36, textAlign: "center", padding: "6px 10px" }}>
                      <input type="checkbox" checked={isChecked}
                        ...
                        disabled={!seleccional} title={tooltip}
